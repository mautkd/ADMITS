...
with open('log_60s_udia.csv') as csv_file:
    
    csv_reader = csv_file.readlines()
    #transforma em lista de 3 elementos
    arquivo = open('saida_log_60s_udia.csv', 'w') 
    cont=0
    col2 = 0
    col3 = 0
    col4 = 0
    ...
    for x in csv_reader:    
        ...
        if(len(lista) ==3) :
                ...
                (value) = lista_value
                (cpu,sys,inter,ctxsw,KBRead,Reads,KBWrit,Writes,KBIn,PktIn,KBOut,PktOut,data,Time)=lista_completo
                hora = int(hora)-3
                if (hora <0):
                    hora = hora*(-1)
                sec2 = sec
                ...
                saida =str(col1)+','+str(col8)+','+str(col9)+','+str(col10)+','+str(col11)+','+str(col12)+','+str(col13)+','+str(col14)+','+str(col15)+','+str(col2)+','+str(col3)+','+str(col4)+','+str(col7)+','+str(col5)+','+str(col6)
                print(saida2)			
                #print(lista_completo,'UNIX TIme convertido = ',unix_time,)
                if(cabecalho ==0):
                     arquivo.write('ID,cpu,sys,inter,ctxsw,KBRead,Reads,KBWrit,Writes,KBIn,PktIn,KBOut,PktOut,Data_original,Unix_time  \n')
                     cabecalho=1
